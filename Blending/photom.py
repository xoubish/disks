...
def make_observation(objid, image_noise = 3.,psf_noise = 1e-3, scale = 1.0):
...
def fit_mof(entry,obs,max_tries = 5, max_chi2 = 25):
    ...
    chi2per = result['chi2per']/result['dof']
    fig.suptitle(f"chi2per: {result['chi2per']:.08}")
    fig.savefig(f"fit_results-{entry['id']}.png")
    plt.close(fig)
...
def get_coordinates(cfile='Coordinates_gan.txt',lowres=False):
    dtype = [('id',np.int),('x1',np.float),('x2',np.float),('y1',np.float),('y2',np.float)]
...
if __name__ == '__main__':
    ...
    for i,entry in enumerate(coords):
        ...
        result = fit_mof(entry,obs,max_tries=10)
