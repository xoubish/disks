...
YL = data['flux_lores'][keepL]
dy = np.hstack([sigma_meas1,sigma_meas2])
dyL = sigma_meas_lores

kernel =   C(1.0, (1e-3, 1e3)) * RBF(100, (1e1, 1e7))
...
ax1.errorbar(X.ravel(),Y/X.ravel(),dy/X.ravel(),elinewidth=0.15,markersize=0.5,fmt='.',zorder=2)
ax1.plot(X.ravel(),Y/X.ravel(),'.',markersize=0.,color='orange',zorder=3)
# -- the model fit
...
ax2.errorbar(XL.ravel(),YL/XL.ravel(),dyL/XL.ravel(),elinewidth=0.15,markersize=0.5,fmt='.',zorder=2)
ax2.plot(XL.ravel(),YL/XL.ravel(),'.',markersize=0.,color='orange',zorder=3)
# -- the model fit
#plt.plot(Xinterp.ravel(),Yinterp+Xinterp.ravel())
