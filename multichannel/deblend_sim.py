...
class Simulation(object):
    ...
    def __init__(self,number_of_images = 100, do_image_plots=False,max_tries=5,max_chi2perdof = 2.,flux_calibration = 5.07224239892, broadband=2):
    ...
    def make_simulated_data(self):
        ...
        for i in range(self.number_of_images):
            hi,i1,i2,lo,gan,psf_hires,psf_lores,data,nums = galblend(gals=2,sel_band=self.broadband,lim_hmag=25,plot_it=self.do_image_plots)
            this_hires_obs = self._make_observation(hi[self.broadband,:,:],psf_hires,image_scale=self.scale_hires,psf_scale=self.scale_psf)
            this_single_obs1 = self._make_observation(i1[self.broadband,:,:],psf_lores,image_scale=self.scale_hires,psf_scale=self.scale_psf)
            this_single_obs2 = self._make_observation(i2[self.broadband,:,:],psf_lores,image_scale=self.scale_hires,psf_scale=self.scale_psf)
            this_lores_obs = self._make_observation(lo[self.broadband,:,:],psf_lores,image_scale=self.scale_lores,psf_scale=self.scale_psf)
