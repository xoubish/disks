...
ker = torch.Tensor(psfh)
ker = ker.permute(2,3,0,1)
kernel = ker.cuda()


parser = argparse.ArgumentParser()
parser.add_argument('--dataset', help='cifar10 | lsun | mnist |imagenet | folder | lfw | fake')
parser.add_argument('--dataroot', default='gals/', help='path to dataset')
...
class Generator(nn.Module):
    ...
    def forward(self, input):
        if input.is_cuda and self.ngpu > 1:
            ...
            ougpug = F.conv2d(output, kernel,padding=int(((kernel.shape[3])-1)/2))
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 100 == 0:
            vutils.save_image(real_cpu,
                    '%s/outputs/real_samples.png' % opt.outf,
                    normalize=True)
            fake = netG(fixed_noise)
            vutils.save_image(fake.detach(),
                    '%s/outputs/fake_samples_epoch_%03d.png' % (opt.outf, epoch),
                    normalize=True)

    # do checkpointing
    torch.save(netG.state_dict(), '%s/outputs/netG_epoch_%d.pth' % (opt.outf, epoch))
    torch.save(netD.state_dict(), '%s/outputs/netD_epoch_%d.pth' % (opt.outf, epoch))
